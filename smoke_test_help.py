...
def test_all_panels():
    ...
    for r in results:
        if r["status"] == "✓ PASS":
            ...
            print(f"     What: {r['what_len']} chars, Why: {r['why_len']} chars, Actions: {r['actions']}")
        else:
            print(f"{r['status']} {r['panel']}")
            if "error" in r:
...
def test_cache_behavior():
    ...
    try:
        ...
        print(f"  Note: Could not clear Redis (using in-memory fallback)")
    
    # Request 1: Cache miss
    print(f"\n  Request 1: Cache MISS (expected)...")
    start = time.time()
    r1 = requests.get(f"{BASE_URL}/agent/describe/{panel_id}", params={"month": test_month})
    t1 = time.time() - start
    ...
    start = time.time()
    r2 = requests.get(f"{BASE_URL}/agent/describe/{panel_id}", params={"month": test_month})
    t2 = time.time() - start
    ...
    start = time.time()
    r3 = requests.get(f"{BASE_URL}/agent/describe/{panel_id}", params={"month": test_month, "refresh": "true"})
    t3 = time.time() - start
...
def test_redis_connectivity():
    ...
    try:
        ...
        print(f"  Note: System will use in-memory cache fallback")
    ...
    for test in tests:
        try:
            ...
            if response.status_code == test["expected"]:
                ...
                print(f"  ✗ FAIL: {test['name']} (expected {test['expected']}, got {response.status_code})")
        except Exception as e:
