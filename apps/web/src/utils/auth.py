...
def _get_keys_and_alg():
    ...
    if alg == "RS256":
        ...
        return priv, pub, alg
    return (
        get_env("AUTH_SECRET", "dev-secret"),
        get_env("AUTH_SECRET", "dev-secret"),
        alg,
    )
...
def create_tokens(email: str, roles: Sequence[str]) -> TokenPair:
    ...
    access_exp = now + dt.timedelta(
        minutes=int(get_env("ACCESS_TOKEN_EXPIRE_MINUTES", "30"))
    )
    refresh_exp = now + dt.timedelta(
        days=int(get_env("REFRESH_TOKEN_EXPIRE_DAYS", "14"))
    )
    base = _base_claims(email, roles)
    access = jwt.encode(
        {**base, "type": "access", "iat": now, "exp": access_exp}, priv, algorithm=alg
    )
    refresh = jwt.encode(
        {**base, "type": "refresh", "iat": now, "exp": refresh_exp}, priv, algorithm=alg
    )
    return TokenPair(
        access_token=access,
...
def decode_token(token: str) -> dict:
    ...
    return jwt.decode(
        ...
        pub,
        algorithms=[alg],
        audience=get_env("AUTH_AUDIENCE", "finance-agent-app"),
    ...
    def _dep(
        ...
        if not set(required).issubset(user_roles):
            raise HTTPException(
                ...
                detail=f"Requires roles: {', '.join(required)}",
            )
        return user
