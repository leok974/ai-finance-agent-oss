...
test.describe('Portal root infrastructure', () => {
  test('portal root exists on page load', async ({ page }) => {
    ...
    // Verify #__LM_PORTAL_ROOT__ div exists (either in HTML or created programmatically)
    const portalRoot = page.locator('#__LM_PORTAL_ROOT__');
    await expect(portalRoot).toHaveCount(1);

    // If created programmatically by ensurePortalRoot(), it will have fixed positioning
    // If in HTML only, it may be static (which is fine - it's a portal target)
    ...
    // Accept either static (HTML-only) or fixed (programmatic)
    expect(['static', 'fixed']).toContain(styles.position);
    
    // If fixed, verify other portal-specific styles
    if (styles.position === 'fixed') {
      expect(styles.pointerEvents).toBe('none');
      expect(parseInt(styles.zIndex)).toBeGreaterThan(2147400000);
    }
  });
