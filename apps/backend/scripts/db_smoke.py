...
with engine.begin() as conn:
    ...
    n = conn.execute(text("SELECT count(*) FROM feedback WHERE created_at IS NULL")).scalar_one()
    print({"feedback_null_created_at": int(n)})

    # 2) Describe feedback table column (portable checks)
    ddl = conn.execute(text("SELECT is_nullable, column_default FROM information_schema.columns WHERE table_name='feedback' AND column_name='created_at'"))
    row = ddl.first()
    if row:
    ...
    # 4) Index presence
    idx = conn.execute(text("SELECT indexname FROM pg_indexes WHERE tablename IN ('feedback','transactions')"))
    names = {r[0] for r in idx}
    print({"has_ix_feedback_created_at": "ix_feedback_created_at" in names, "has_ix_transactions_date": "ix_transactions_date" in names})
