def test_chart_category_endpoint(client, db_session):
    ...
    db_session.add(Transaction(date=date(2025,8,3), category="Groceries", amount=-100))
    db_session.add(Transaction(date=date(2025,9,3), category="Groceries", amount=-120))
    db_session.commit()
