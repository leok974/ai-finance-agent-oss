...
def test_sum_last_month(client, db_session):
    ...
    seed(db)
    r = client.post(
        "/agent/txns_query", json={"q": "How much did I spend at Starbucks last month?"}
    )
...
def test_top_merchants_range(client, db_session):
    ...
    r = client.post(
        "/agent/txns_query",
        json={"q": "top 2 merchants between 2025-08-01 and 2025-08-31"},
    )
    assert r.status_code == 200
    items = r.json()["result"]
    assert len(items) <= 2
...
def test_list_with_amount_filter(client, db_session):
    ...
    seed(db)
    r = client.post(
        "/agent/txns_query", json={"q": "show groceries over $40 in August 2025"}
    )
    assert r.status_code == 200
    data = r.json()
...
def test_csv_export_endpoint(client, db_session):
    ...
    assert body.splitlines()[0].startswith(
        "id,date,merchant,category,amount,description,merchant_canonical"
    )
