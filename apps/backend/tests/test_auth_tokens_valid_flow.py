...
def test_auth_me_200_and_refresh_with_auto_csrf_detection(client, monkeypatch):
    ...
    if r_me.status_code != 200:
        ...
        if r_me.status_code in (401, 404):
            pytest.xfail(
                f"User not provisioned for token subject; status={r_me.status_code}"
            )
        assert (
            r_me.status_code == 200
        ), f"{auth_me} expected 200 with valid token, got {r_me.status_code}: {r_me.text}"
    ...
    if not auth_refresh:
        pytest.skip(
            "No auth refresh endpoint variant available (/api/auth/refresh or /auth/refresh)"
        )
    ...
    cookie_candidates.extend(
        [
            c
            for c in ("access_token", "access", "jwt", "session")
            if c not in cookie_candidates
        ]
    )
    ...
    for name in cookie_candidates:
        last = client.post(
            auth_refresh, cookies={name: access}, headers={CSRF_HEADER: csrf_value}
        )
    ...
    if not refreshed_any:
        pytest.xfail(
            f"Refresh requires a server-issued refresh token; last status={getattr(last,'status_code',None)}"
        )
