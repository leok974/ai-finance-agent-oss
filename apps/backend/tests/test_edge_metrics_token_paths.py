...
def test_edge_metrics_allows_when_no_token_configured(monkeypatch):
    ...
    r = client.post("/api/metrics/edge", json={"csp_policy_len": 1, "csp_policy_sha256": "x"})
...
def test_edge_metrics_requires_matching_token(monkeypatch):
    ...
    # Wrong token -> 401
    r_bad = client.post("/api/metrics/edge", headers={"X-Edge-Token": "nope"}, json={"csp_policy_len": 1, "csp_policy_sha256": "x"})
    assert r_bad.status_code == 401

    # Correct token (with surrounding whitespace to test .strip()) -> 204
    r_ok = client.post("/api/metrics/edge", headers={"X-Edge-Token": " test-token "}, json={"csp_policy_len": 2, "csp_policy_sha256": "y"})
