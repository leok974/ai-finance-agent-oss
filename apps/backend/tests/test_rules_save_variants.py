...
def test_json_fallback_path(client: TestClient, payload_rule, use_tmp_fallback, monkeypatch):
    """
    ...
    monkeypatch.setattr(rs, "build_ack", lambda s, count=1, **k: f"ack:{s}:{count}", raising=False)
...
def test_db_failure_fallback_to_json(client: TestClient, payload_rule, use_tmp_fallback, monkeypatch):
    """
