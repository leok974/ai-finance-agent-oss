...
def test_rules_save_deep_merge_and_replace_plan(auth_client, monkeypatch):
    ...
    batch = {
        "rules": [
            {"id": "seed-1", "name": "Groceries+", "pattern": "aldi|kroger|wholefoods", "category": "groceries", "enabled": True},
            {"id": "new-3",  "name": "Dining",     "pattern": "chipotle|pizza",         "category": "restaurants", "enabled": True},
            {"id": "dup-4",  "name": "CoffeeA",    "pattern": "coffee",                 "category": "restaurants", "enabled": True},
            {"id": "dup-4",  "name": "CoffeeB",    "pattern": "espresso",               "category": "restaurants", "enabled": True},
        ],
        "dry_run": True,
        "replace": False,
    }
    r1 = _post(auth_client, batch); _no_500(r1)
    assert r1.status_code in (200, 201, 202, 204, 400, 422)

    rep = {
        "rules": [
            {"id": "only-1", "name": "OnlyOne", "pattern": "unique", "category": "misc", "enabled": True}
        ],
        "dry_run": True,
        "replace": True,
    }
    r2 = _post(auth_client, rep); _no_500(r2)
