...
@pytest.mark.skipif(
    not _has("check_crypto_via_env"), reason="check_crypto_via_env not present"
)
...
@pytest.mark.skipif(
    not _has("check_llm_health_sync"), reason="check_llm_health_sync not present"
)
...
@pytest.mark.skipif(
    not all(_has(n) for n in ("status_snapshot",)), reason="status_snapshot not present"
)
def test_status_snapshot_aggregates_components(monkeypatch):
    if _has("check_db"):
        monkeypatch.setattr(
            su, "check_db", lambda *a, **k: su.DBStatus(ok=True), raising=True
        )
    if _has("check_crypto_via_env"):
        monkeypatch.setattr(
            su,
            "check_crypto_via_env",
            lambda: su.CryptoStatus(ok=True, mode="kms"),
            raising=True,
        )
    if _has("check_llm_health_sync"):
        monkeypatch.setattr(
            ...
            lambda: su.LLMStatus(ok=False, error="down"),
            raising=True,
        )
