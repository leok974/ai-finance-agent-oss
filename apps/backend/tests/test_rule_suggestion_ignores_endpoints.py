def test_rule_suggestion_ignores_crud(client, db_session):
    ...
    # add
    r1 = client.post(
        "/rules/suggestions/ignores",
        json={"merchant": "Starbucks", "category": "Dining out"},
    ).json()
    ...
    assert any(
        i["merchant"] == "Starbucks" and i["category"] == "Dining out"
        for i in r2["ignores"]
    )
    ...
    assert all(
        not (i["merchant"] == "Starbucks" and i["category"] == "Dining out")
        for i in r3["ignores"]
    )
