...
@pytest.mark.xfail(reason="coverage check not implemented yet; placeholder to prevent regressions when added")
def test_existing_rule_coverage_skips_suggestion(db_session, monkeypatch):
    ...
    t = Transaction(date=d, merchant="Cafe X", description="coffee", amount=-5.50, category=None, raw_category=None, account=None, month=d.strftime("%Y-%m"))
    db_session.add(t)
    db_session.flush()
    db_session.add(Feedback(txn_id=t.id, label="Coffee", source="accept"))
    db_session.add(Rule(pattern=rs.canonicalize_merchant("Cafe X"), target="merchant", category="Coffee", active=True))
    db_session.commit()
