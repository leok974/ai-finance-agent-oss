...
AMOUNT_RE = re.compile(
    r"(?:(?:over|above|greater than|>)\s*\$?\s*(\d+(?:\.\d+)?))|(?:\$\s*(\d+(?:\.\d+)?))",
    re.I,
)
MONTH_WORDS = {
    "january": "01",
...
def agent_chat(req: ChatReq):
    ...
    try:
        if (
            any(k in text for k in ["summary", "overview", "net", "income", "spend"])
            and month
        ):
            ...
            cats = sorted((out.get("categories") or {}).items(), key=lambda x: -x[1])[
                :5
            ]
            if cats:
                parts.append(
                    "Top categories: " + ", ".join(f"{k} (${v:.0f})" for k, v in cats)
                )
            return {"reply": "\n".join(parts)}
        ...
        if any(
            ...
            if not txns:
                return {
                    "reply": f"No transactions found {('in '+month) if month else ''} over ${args['min_amount']:.0f}."
                }
            lines = [
                f"Top {len(txns)} large transactions{(' in '+month) if month else ''} (≥ ${args['min_amount']:.0f}):"
            ]
            for t in txns:
                lines.append(
                    f"- {t['date']} — {t.get('merchant') or t.get('description') or 'Unknown'} — ${abs(t['amount']):,.2f} [{t.get('category','Unknown')}]"
                )
            return {"reply": "\n".join(lines)}
        ...
        if "budget" in text or "over budget" in text or "under budget" in text:
            if not month:
                return {
                    "reply": "Which month should I analyze for budgets? (e.g., 2023-12)"
                }
            ...
            if overs:
                reply.append(
                    "Over budget: "
                    + ", ".join(
                        f"{x['category']} (+${x['over']:.0f})" for x in overs[:5]
                    )
                )
            return {"reply": "\n".join(reply)}
        ...
        if "merchant" in text and month:
            ...
            return {
                "reply": "Top merchants: "
                + ", ".join(f"{x['merchant']} (${x['amount']:.0f})" for x in top)
            }
        ...
        if "category" in text and month:
            ...
            return {
                ...
                + ", ".join(f"{x['name']} (${x['amount']:.0f})" for x in top)
            }

        # fallback concise echo
        return {
            "reply": f"I’m here. Try asking, e.g., 'Summarize {month or '2023-12'}' or 'Large transactions over $500'."
        }
