...
try:
    ...
    Exception
):  # pragma: no cover  # optional dependency fallback; see docs/coverage-ratchet.md
...
_FALLBACK_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "data", "rules_saved.json")
)
...
@router.post(
    "/save", response_model=SaveRuleResponse, dependencies=[Depends(csrf_protect)]
)
async def save_rule(
    ...
    if rule is None and payload.scenario:
        rule = RuleInput(
            ...
            when={
                ...
                **({"month": payload.month} if payload.month else {}),
            },
            then=RuleThen(),
        )
    if rule is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Missing rule or scenario"
        )
    ...
    if create_rule_db and db is not None:
        try:
            ...
            display = getattr(created, "display_name", None) or (
                rule.name or f"Rule {rid[:8]}"
            )
            ack = build_ack("rules.save", 1)
            resp = SaveRuleResponse(
                ok=True, id=rid, display_name=display, source="db", ack=ack
            )
        except Exception:
            doc = _load_json()
            rid = str(uuid.uuid4())
            ...
            resp = SaveRuleResponse(
                ok=True, id=rid, display_name=display, source="json", ack=ack
            )
    else:
        doc = _load_json()
        rid = str(uuid.uuid4())
