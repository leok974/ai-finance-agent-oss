...
@router.get("/list", response_model=BudgetListResp, summary="List persisted budgets (category->amount)")
...
@router.get("/check", response_model=List[BudgetCheckItem], summary="Compare current month spend vs budget caps")
...
@router.get("/recommendations", response_model=BudgetRecsResp, summary="Compute per-category budget suggestions from history")
...
def set_budget(req: BudgetSetReq, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "budget": {
            ...
            "updated_at": str(getattr(obj, "updated_at", None)) if hasattr(obj, "updated_at") else None,
        },
    }
...
def set_temp_budget(req: TempBudgetReq, db: Session = Depends(get_db)):
    ...
    return {"ok": True, "temp_budget": {"month": month, "category": key[1], "amount": TEMP_BUDGETS[key]}}


@router.get(
    "/temp",
    response_model=TempBudgetListResp,
    summary="List temporary budget overlays"
)
def list_temp_budgets(month: str | None = Query(None)):
    ...
    items = [{"month": mm, "category": cat, "amount": amt} for (mm, cat), amt in TEMP_BUDGETS.items() if mm == m]
...
def clear_temp_budget(category: str = Path(..., min_length=1), month: str | None = Query(None)):
    """
    ...
    return {"ok": True, "deleted": {"month": m, "category": key[1], "amount": amt, "existed": existed}}
...
def clear_temp_budget_budgets(category: str = Path(..., min_length=1), month: str | None = Query(None)):
...
def read_budgets(
    month: Optional[str] = Query(None, description='Month key "YYYY-MM". Defaults to current.'),
    merge_temp: bool = Query(False, description="If true, merge in-memory temp overlays."),
    include: Optional[str] = Query(None, description="Comma-separated categories to include."),
    ...
    exc = set([s.strip() for s in exclude.split(",") if s.strip()]) if exclude else set()
