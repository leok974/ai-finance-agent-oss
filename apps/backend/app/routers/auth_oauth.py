...
async def github_start(request: Request):
    return await oauth.github.authorize_redirect(
        request, absolute_url(request, "/auth/github/callback")
    )
...
async def google_start(request: Request):
    return await oauth.google.authorize_redirect(
        request, absolute_url(request, "/auth/google/callback"), prompt="consent"
    )
...
async def _finalize(
    provider: str, provider_user_id: str, email: str | None, db: Session
):
    link = (
    ...
    if link:
        ...
        if not user:
            user = User(
                ...
                password_hash=hash_password(provider_user_id),
            )
            db.add(user)
            db.commit()
            db.refresh(user)
    ...
    redirect = os.environ.get(
        ...
        get_env("OAUTH_POST_LOGIN_REDIRECT", "http://localhost:5173/app"),
    )
