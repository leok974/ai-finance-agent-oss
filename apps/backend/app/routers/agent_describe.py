...
def _validate_month_or_422(month: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail=f"Invalid month format: {month}. Expected YYYY-MM"
        )
...
def describe_panel(
    ...
    panel_id: str = ...,
    month: str = Query(
        ..., regex=r"^\d{4}-\d{2}$", description="Month in YYYY-MM format"
    ),
    ...
    # Check cache first (unless refresh requested) - scoped by user_id
    cache_key = f"help:u{user_id}:{panel_id}:{month}"
    if not refresh:
    ...
    if METRICS_AVAILABLE:
        lm_help_requests_total.labels(
            panel_id=panel_id, cache="miss" if not refresh else "refresh"
        ).inc()

    # Route to appropriate explainer
    if panel_id == "charts.month_merchants":
        data = explain_month_merchants(db, user_id, month)
    elif panel_id == "charts.month_categories":
        data = explain_month_categories(db, user_id, month)
    elif panel_id == "charts.daily_flows":
        data = explain_daily_flows(db, user_id, month)
    elif panel_id == "charts.month_anomalies":
        data = explain_month_anomalies(db, user_id, month)
    elif panel_id == "charts.insights_overview":
        data = explain_insights_overview(db, user_id, month)
    else:
        raise HTTPException(
            status_code=404,
    ...
    if not data.get("insights") or (
        isinstance(data.get("insights"), list) and len(data["insights"]) == 0
    ):
        # Check if it's truly no data
        what = data.get("what", "")
        if "Txns=0" in what or "No significant spending" in what:
            data["why"] = (
                f"No transactions found for {month}. Upload a CSV or check date filters."
            )
            data["actions"] = [
