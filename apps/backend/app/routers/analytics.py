...
def kpis(payload: dict = Body(default={}), db: Session = Depends(get_db)):
    ...
    lookback_months = int(payload.get("lookback_months", 6)) if isinstance(payload, dict) else 6
...
def forecast_cashflow(payload: dict = Body(default={}), db: Session = Depends(get_db)):
    ...
    return svc.forecast_cashflow(db, month=month, horizon=horizon, model=model, alpha=alpha)
