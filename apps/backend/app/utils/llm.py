...
# --- GPU Request Guardrails --------------------------------------------------
# Prevent concurrent GPU inference requests that can cause timeouts/OOM
_gpu_request_lock = threading.Lock()


class LLMQueueFullError(Exception):
    """Raised when LLM GPU is busy and request cannot be processed immediately."""
...
_fallback_provider: ContextVar[Optional[str]] = ContextVar(
    "_fallback_provider", default=None
)
...
def _post_chat(
    base: str, key: str, payload: dict, timeout: int = 60, *, _attempt: int = 0
) -> dict:
...
def call_llm(
    *,
    ...
    - provider='openai'  -> https://api.openai.com/v1 (requires real OPENAI_API_KEY)
        ...
        GPU Guardrails:
        - Uses a process-wide mutex to prevent concurrent GPU requests
        - Returns "queue full" message if GPU is busy to prevent timeouts
    """

    # GPU Request Guardrails: Check if inference is already in progress
    if not _gpu_request_lock.acquire(blocking=False):
        # GPU is busy with another request - raise exception for HTTP 429 handling
        try:
            rid = get_request_id() or "-"
            _log.info("LLM:queue_full rid=%s msg='GPU busy, returning queue full'", rid)
        except Exception:
            pass
        raise LLMQueueFullError(
            "GPU is currently processing another request. Please retry in a moment."
        )

    try:
        # All LLM logic now runs within the mutex
        return _call_llm_impl(
            model=model, messages=messages, temperature=temperature, top_p=top_p
        )
    finally:
        # Always release the lock
        _gpu_request_lock.release()
...
def _call_llm_impl(
    *,
    ...
    """
    Internal implementation of call_llm that runs within GPU mutex.
    """
    ...
    try:
        rid = get_request_id() or "-"
        _log.info(
            "LLM:call start rid=%s base=%s model=%s msgs=%d",
            rid,
            base,
            model,
            len(messages),
    ...
    def _try_ollama_native() -> Optional[dict]:
        ...
        try:
            ...
            r = requests.post(
                f"{root}/api/chat", json=chat_body, headers=headers, timeout=20
            )
            ...
            r2 = requests.post(
                f"{root}/api/generate", json=gen_body, headers=headers, timeout=20
            )
    ...
    try:
        ...
        try:
            ...
            _tmp = ((data.get("choices") or [{}])[0].get("message") or {}).get(
                "content"
            ) or ""
            _log.info(
                "LLM:primary ok chars=%d rid=%s", len(_tmp), get_request_id() or "-"
            )
        except Exception:
            pass
    except (
        ...
        is_timeout = isinstance(err, (requests.exceptions.ConnectTimeout)) or (
            isinstance(getattr(err, "response", None), requests.Response)
            and False  # placeholder for future classification
        )
        if (
            is_timeout
            and LLM_INITIAL_RETRY > 0
            and (time.time() - _PROCESS_START_TS) <= LLM_WARM_WINDOW_S
        ):
            try:
                backoff = 0.3 + random.random() * 0.4
                time.sleep(backoff)
                data = _post_chat(
                    base, key, payload, timeout=int(LLM_READ_TIMEOUT), _attempt=1
                )
                err = None  # type: ignore
            except Exception as retry_err:  # pragma: no cover - defensive
                err = retry_err  # type: ignore
                _log.debug(
                    "llm.retry.failed warm_window backoff=%s err=%s",
                    round(backoff, 2),
                    retry_err,
                )
        ...
        if "data" in locals():
            ...
            try:
                ...
                try:
                    ...
                    _log.info(
                        "LLM:fallback openai ok chars=%d rid=%s",
                        len(_tmp),
                        get_request_id() or "-",
                    )
                except Exception:
                ...
                try:
                    ...
                    pass
                return (
                    "The model backend is unavailable right now. Please try again shortly.",
                    [],
                )
        else:
            ...
            if warm_window and is_timeout:
                try:
                    ...
                    pass
                return (
                    "[model_warming] The model is still loading; please retry shortly.",
                    [],
                )
            try:
                _log.warning(
                    "LLM:transient failure no-fallback returning friendly stub (%s)",
                    type(err),
                )
            except Exception:
                pass
...
def list_models() -> Dict[str, List[Dict[str, str]]]:
    ...
    if provider == "ollama":
        ...
        models = [
            {"id": m.get("name")} for m in data.get("models", []) if m.get("name")
        ]
        return {
            "provider": provider,
            "default": settings.DEFAULT_LLM_MODEL,
...
def invoke_llm_with_optional_stub(
    *,
    ...
    model_norm = (
        alias_map.get(requested_model, requested_model) if requested_model else None
    )
