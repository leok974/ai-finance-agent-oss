...
def latest_month_from_data(db: Session) -> Optional[str]:
    ...
    if unlabeled_row:
        try:
            ...
            mval = (
                unlabeled_row[0]
                if isinstance(unlabeled_row, (list, tuple))
                else unlabeled_row
            )
        if mval:
            return str(mval)
    ...
    try:
        ...
        m, d = row[0], (
            row[1] if isinstance(row, (list, tuple)) and len(row) >= 2 else (None, None)
        )
    if m:
        return str(m)
...
def _rule_matches_txn(rule: Rule, txn: Transaction) -> bool:
    ...
    if rule.pattern:
        ok = ok and (
            rule.pattern.lower() in (txn.merchant or "").lower()
...
def find_candidates(
    db: Session, month: str, rules: Iterable[Rule]
) -> List[Transaction]:
    """
...
def apply_all_active_rules(
    ...
    for t in txns:
        ...
        applied += 1
        details.append(
            {"id": t.id, "rule_id": matched.id, "category": matched.category}
        )
