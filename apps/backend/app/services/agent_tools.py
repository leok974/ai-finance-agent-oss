...
from app.services.agent_detect import (
    detect_budget_recommendation,
    extract_months_or_default,
    Detector,
)
    ...
    if name == "categorize_txn":
        ...
        for t in app.state.txns:
            if t["id"] == tid:
                ...
                return {
                    "ok": True,
                    "message": f"Categorized transaction {tid} as {cat}",
                }
        ...
        total_spent = sum(
            abs(float(t["amount"])) for t in month_txns if float(t["amount"]) < 0
        )
        total_income = sum(
            float(t["amount"]) for t in month_txns if float(t["amount"]) > 0
        )
        ...
        for month in recent_months:
            ...
            if category_filter:
                month_txns = [
                    t for t in month_txns if t.get("category") == category_filter
                ]
        ...
        return {
            ...
            "avg_monthly": (
                round(sum(t["spent"] for t in trends) / len(trends), 2) if trends else 0
            ),
        }
        ...
        app.state.budget_rules[category] = {"limit": limit, "period": period}
...
def route_to_tool(user_text: str, db: Session) -> Optional[Dict[str, Any]]:
    """
    Returns a dict with:
      - mode: one of ["nl_txns", "charts.summary", "charts.flows", "charts.merchants", "charts.categories", "report.link", "budgets.read", "quick_recap"]
    ...
    # 0) Quick recap — conversational monthly summary
    if any(
        phrase in text_low
        for phrase in [
            "quick recap",
        ...
        from datetime import datetime
        from app.services.agent.quick_recap import human_recap, to_conversational_text

        try:
            summary = human_recap(db, now=datetime.utcnow())
            text = to_conversational_text(summary)
            return {
                "mode": "quick_recap",
                "filters": {"month": summary["month"]},
                "result": summary,
                "message": text,
                # No stub flag - this is a complete response
            }
            ...
            return {
                ...
                "filters": {},
                "result": {},
                "message": f"I couldn't generate a recap right now: {str(e)}",
            }
    ...
    if det.detect_anomalies(user_text):
        ...
        if not anomalies_list:
            ...
            month_label = (
                _extract_month(user_text) or latest_month_str(db) or "this period"
            )
    ...
    if hit:
        ...
        if mode == "analytics.forecast":
            ...
            if not data.get("ok", True) or not data.get("forecast"):
                return _no_data_response(
                    ...
                    tool_label="Forecast",
                    tips=[
                        "Use Insights: Expanded (last 60 days)",
        ...
        if mode == "analytics.whatif":
            ...
            return {
                "mode": mode,
                "filters": {"month": payload.get("month")},
                "args": args,
                "result": data,
            }
    ...
    if any(
        ...
        k in text_low
        for k in ["top merchants", "merchants breakdown", "merchant spend"]
    ):
        charts_kind = "merchants"
    elif any(
        k in text_low for k in ["categories", "category breakdown", "by category"]
    ):
    ...
    if charts_kind:
        ...
        if charts_kind == "merchants":
            ...
            return {
                "mode": "charts.merchants",
                "filters": {"month": month},
                "result": data.get("merchants", []),
            }
        if charts_kind == "categories":
            data = get_month_categories(db, month)
            return {
                "mode": "charts.categories",
                "filters": {"month": month},
                "result": data,
            }
    ...
    if any(
        k in text_low for k in ["export", "download", "report", "excel", "xlsx", "pdf"]
    ):
        month = _extract_month(user_text) or latest_month_str(db)
        qs = []
        if month:
            qs.append(f"month={month}")
        include_tx = ("include transaction" in text_low) or (
            "with transactions" in text_low
        )
        kind = (
            "excel"
            if ("excel" in text_low or "xlsx" in text_low)
            else ("pdf" if "pdf" in text_low else "excel")
        )
        if kind == "excel":
            if include_tx:
    ...
    if any(
        k in text_low
        for k in ["budget", "over budget", "under budget", "remaining budget"]
    ):
        return {
            "mode": "budgets.read",
            "filters": {},
