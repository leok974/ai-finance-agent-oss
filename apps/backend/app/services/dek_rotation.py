...
from app.orm_models import (
    ...
    EncryptionKey,
    EncryptionSettings,
)  # adjusted to actual orm_models
from app.core.crypto_state import (
    get_dek_for_label,
    purge_dek_cache,
)  # canonical DEK retrieval
...
try:  # pragma: no cover - metrics optional
    ...
    _ROTATE_SCANNED = Counter(
        "crypto_rotation_scanned_total", "Rows scanned during DEK rotation"
    )
    _ROTATE_PROCESSED = Counter(
        "crypto_rotation_processed_total", "Rows re-encrypted during DEK rotation"
    )
    _ROTATE_FAILED = Counter(
        "crypto_rotation_decrypt_fail_total", "Decrypt failures during DEK rotation"
    )
    _ROTATE_OK = Counter(
        "crypto_rotation_decrypt_ok_total",
        "Successful decrypt operations during rotation",
    )
    _ROTATE_REMAINING = Gauge(
        "crypto_rotation_remaining",
        "Remaining rows under source label needing rotation",
    )
    _ROTATE_LAST_BATCH = Gauge(
        "crypto_rotation_last_batch_size", "Row count processed in last rotation batch"
    )
    ...
    _ROTATE_SCANNED = _ROTATE_PROCESSED = _ROTATE_FAILED = _ROTATE_OK = (
        _ROTATE_REMAINING
    ) = _ROTATE_LAST_BATCH = _ROTATE_BATCH_LATENCY = _ROTATE_FAIL_FIELD = (
        _ROTATE_FINALIZE
    ) = None
...
def rotation_status(db: Session) -> Dict[str, Any]:
    ...
    if rotating is None:
        ...
        row = db.execute(
            select(EncryptionKey.label)
            .where(EncryptionKey.label.like("rotating::%"))
            .order_by(EncryptionKey.created_at.desc())
        ).first()
        rotating = row.label if row else None
    done = (
        db.query(Transaction).filter(Transaction.enc_label == rotating).count()
        if rotating
        else 0
    )
    total = (
        ...
        .filter(
            Transaction.enc_label.in_([active, rotating])
            if rotating
            else Transaction.enc_label == active
        )
        .count()
    )
...
def run_rotation(
    ...
    *,
    ...
    q = (
        ...
        .filter(payload_filter)
        .filter(
            (Transaction.enc_label == None) | (Transaction.enc_label == source_label)
        )  # noqa: E711
    ...
    if _ROTATE_SCANNED:
        try:
            ...
            remaining = (
                max(0, (count_source_label - processed))
                if not dry_run
                else count_source_label
            )
    ...
    if eta_seconds is not None and prev_eta is not None:
        try:
            pct_threshold = float(
                os.getenv("ROTATION_ETA_WARN_INCREASE_PCT", "0.25")
            )  # 25%
        except Exception:
            pct_threshold = 0.25
        try:
            ...
            min_delta = 30.0
        if eta_seconds - prev_eta >= min_delta and eta_seconds > prev_eta * (
            1 + pct_threshold
        ):
...
def finalize_rotation(db: Session, *, target_label: str) -> Dict[str, Any]:
    ...
    db.flush()
    # Relabel rows
    db.query(Transaction).filter(Transaction.enc_label == rotating).update(
        {Transaction.enc_label: rotating}, synchronize_session=False
    )
    db.commit()
