...
def ctx_month_summary(db: Session, month: str) -> Dict[str, Any]:
    ...
    return {"month": month, "total_outflows": float(total_out), "total_inflows": float(total_in)}
...
def ctx_top_merchants(db: Session, month: str) -> List[Dict[str, Any]]:
    ...
    ).filter(Transaction.month == month).group_by(Transaction.merchant).order_by(desc("total")).limit(50).all()
    return [{"merchant": r[0], "total": float(r[1]), "count": int(r[2])} for r in rows if r[0]]
