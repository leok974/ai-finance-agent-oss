...
def list_budget_rules(db: Session) -> List[Dict]:
    ...
    for b in rows:
        rules.append({
            ...
            "active": True,
            "created_at": str(getattr(b, "created_at", None)) if getattr(b, "created_at", None) else None,
            "updated_at": str(getattr(b, "updated_at", None)) if getattr(b, "updated_at", None) else None,
        })
    return rules
