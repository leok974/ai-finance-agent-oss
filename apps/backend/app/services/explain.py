...
def _top_merchants(db: Session, user_id: int, yyyymm: str, limit: int = 7):
    ...
    query = text(
        ...
        FROM transactions
        WHERE user_id = :user_id AND date >= :start AND date < :end AND amount < 0
        GROUP BY merchant
        ORDER BY total ASC
        LIMIT :limit
    """
    ...
    result = db.execute(
        query, {"user_id": user_id, "start": start, "end": end, "limit": limit}
    )
    return [(row.merchant, abs(row.total)) for row in result]


def _daily_spikes(db: Session, user_id: int, yyyymm: str, top_n: int = 3):
    ...
    result = db.execute(
        query, {"user_id": user_id, "start": start, "end": end, "top_n": top_n}
    )
    return [(row.date, abs(row.daily_total)) for row in result]


def explain_month_merchants(db: Session, user_id: int, yyyymm: str) -> Explain:
    """Generate contextual explanation for charts.month_merchants panel."""
    merchants = _top_merchants(db, user_id, yyyymm, limit=7)
    spikes = _daily_spikes(db, user_id, yyyymm, top_n=3)

    # Build "what" summary
    if merchants:
        total_spend = sum(m[1] for m in merchants)
        top3_share = (
            sum(m[1] for m in merchants[:3]) / total_spend if total_spend > 0 else 0
        )
    ...
    if rag_why:
        ...
        insights = []
        if any(
            kw in " ".join(merchant_names)
            for kw in ["utility", "electric", "gas", "water"]
        ):
            insights.append("Utilities present - recurring monthly bills")
        if any(
            kw in " ".join(merchant_names)
            for kw in ["doordash", "uber", "grubhub", "delivery"]
        ):
            insights.append(
                "Food delivery detected - discretionary spending opportunity"
            )
        if any(
            kw in " ".join(merchant_names) for kw in ["amazon", "walmart", "target"]
        ):
            insights.append("E-commerce/retail - check for impulse purchases")
        if any(
            kw in " ".join(merchant_names)
            for kw in ["subscription", "netflix", "spotify", "adobe"]
        ):
        ...
        if len(merchants) >= 5 and merchants[0][1] > sum(m[1] for m in merchants) * 0.4:
            insights.append(
                "High concentration in top merchant - single vendor dependency"
            )
    ...
    if merchants:
        actions.insert(
            0,
            f"Create a rule to auto-categorize {merchants[0][0]} (Unknowns → Promote to rule)",
        )
...
def _month_range(db: Session, user_id: int, yyyymm: str) -> Tuple[int, float, float]:
...
def _categories_breakdown(db: Session, user_id: int, yyyymm: str, limit=7):
    """
    ...
    category_col = (
        ...
        else func.coalesce(None, "Unknown")
    )
...
def _daily_in_out_series(db: Session, user_id: int, yyyymm: str, top_n_spikes=3):
...
def explain_month_categories(db: Session, user_id: int, yyyymm: str) -> Dict[str, Any]:
    """Generate contextual explanation for charts.month_categories panel."""
    cats = _categories_breakdown(db, user_id, yyyymm, limit=7)
    n_txn, total_out, total_in = _month_range(db, user_id, yyyymm)

    top = cats[0] if cats else None
    what_bits = [f"Txns={n_txn}, Out=${total_out:.0f}, In=${total_in:.0f}"]
    if top:
        what_bits.append(
            f"Top category: {top['category']} (${top['spend']:.0f}, {top['count']} txns)"
        )
    if len(cats) > 1:
    ...
    context_bullets = [
        ...
        *what_bits,
        "Categories="
        + ", ".join(f"{c['category']}(${c['spend']:.0f})" for c in cats[:5]),
    ]
...
def explain_daily_flows(db: Session, user_id: int, yyyymm: str) -> Dict[str, Any]:
    """Generate contextual explanation for charts.daily_flows panel."""
    series, spikes = _daily_in_out_series(db, user_id, yyyymm, top_n_spikes=3)
    n_txn, total_out, total_in = _month_range(db, user_id, yyyymm)
    ...
    if spikes:
        what_bits.append(
            "Largest outflow days: "
            + ", ".join(f"{s['date']} (${s['outflow']:.0f})" for s in spikes)
        )
...
def explain_month_anomalies(db: Session, user_id: int, yyyymm: str) -> Dict[str, Any]:
    """Generate contextual explanation for charts.month_anomalies panel."""
    # Reuse spikes + outlier merchants/categories as "anomalies"
    series, spikes = _daily_in_out_series(db, user_id, yyyymm, top_n_spikes=5)
    n_txn, total_out, total_in = _month_range(db, user_id, yyyymm)
    ...
    context_bullets = [
        ...
        what,
        (
            "Spike dates=" + ", ".join(s["date"] for s in spikes)
            if spikes
            else "No spikes"
        ),
    ]
...
def explain_insights_overview(db: Session, user_id: int, yyyymm: str) -> Dict[str, Any]:
    """Generate contextual explanation for charts.insights_overview panel."""
    cats = _categories_breakdown(db, user_id, yyyymm, limit=5)
    merchants = _top_merchants(db, user_id, yyyymm, limit=5)
    series, spikes = _daily_in_out_series(db, user_id, yyyymm, top_n_spikes=2)
    n_txn, total_out, total_in = _month_range(db, user_id, yyyymm)
