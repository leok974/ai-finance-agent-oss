...
def iter_txn_rows(conn, batch: int) -> Iterable[Tuple[int, str | None]]:
    ...
    while True:
        rows = conn.execute(
            text(
                "SELECT id, merchant FROM transactions ORDER BY id ASC LIMIT :lim OFFSET :off"
            ),
...
def main():
    ap = argparse.ArgumentParser(
        description="Recompute transactions.merchant_canonical"
    )
    ...
    ap.add_argument(
        "--only-missing",
        action="store_true",
        help="Update rows where merchant_canonical is NULL/empty only",
    )
    args = ap.parse_args()
    ...
    print(
        f"[recanonicalize] batch={args.batch} dry_run={args.dry_run} only_missing={args.only_missing}"
    )
    try:
        if args.only_missing:
            q = text(
                "SELECT id, merchant FROM transactions WHERE merchant_canonical IS NULL OR merchant_canonical = '' ORDER BY id"
            )
        ...
        for id_, merchant in iterable:
            ...
            if args.only_missing:
                ...
                current = conn.execute(
                    text("SELECT merchant_canonical FROM transactions WHERE id=:id"),
                    {"id": id_},
                ).scalar()
            ...
            if do_update:
                ...
                if not args.dry_run:
                    conn.execute(
                        text(
                            "UPDATE transactions SET merchant_canonical=:mc WHERE id=:id"
                        ),
                        {"mc": mc, "id": id_},
                    )
        ...
        print(
            f"[recanonicalize] scanned={scanned} updates={updates} dry_run={args.dry_run}"
        )
    finally:
        session.close()
