...
def _unwrap_dek(dek_wrap_nonce: bytes | None, dek_wrapped: bytes) -> bytes:
    ...
    if not kek:
        raise RuntimeError(
            "KEK env not set (MASTER_KEK_B64 / ENCRYPTION_MASTER_KEY_BASE64)"
        )
...
def finalize_rotation(new_label: str) -> Tuple[str, str]:
    ...
    if not out.get("ok"):
        ...
        db.execute(
            text("UPDATE encryption_keys SET label='active' WHERE label=:nl"),
            {"nl": new_label},
        )
        db.execute(
            text("UPDATE transactions SET enc_label='active' WHERE enc_label=:nl"),
            {"nl": new_label},
        )
        db.commit()
...
def rotation_status(new_label: Optional[str] = None) -> dict:
    ...
    if not new_label:
        row = db.execute(
            text(
                "SELECT label FROM encryption_keys WHERE label LIKE 'rotating::%' ORDER BY created_at DESC LIMIT 1"
            )
        ).first()
        if not row:
    ...
    return {
        "label": new_label,
        "total_cipher_rows": int(tot),
        "done": int(done),
        "remaining": int(tot - done),
    }


def run_rotation(
    new_label: str, batch_size: int = 1000, max_batches: int = 0, dry_run: bool = False
) -> dict:
    ...
    diagnostics = (
        (svc_out or {}).get("diagnostics", {}) if isinstance(svc_out, dict) else {}
    )
    svc_batches = diagnostics.get("batches", [])
    logical_batches = len(svc_batches) or (
        1 if status.get("total_cipher_rows", 0) else 0
    )
    ...
    status.update(
        {
            "processed_this_run": (
                svc_out.get("processed", 0) if isinstance(svc_out, dict) else 0
            ),
