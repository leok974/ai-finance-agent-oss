...
# Use SQLite-compatible date arithmetic for tests
SQL = """
SELECT f.txn_id, f.ts_month, f.amount, f.abs_amount, f.merchant, f.mcc, f.channel,
       ...
       f.feat_p2p_flag, f.feat_p2p_large_outflow,
       l.label
FROM ml_features f
JOIN transaction_labels l ON l.txn_id = f.txn_id
WHERE f.ts_month >= date('now', '-180 days')
  AND l.label IS NOT NULL
"""


def load_dataframe(limit: int | None = None, connection=None) -> pd.DataFrame:
    """Load training data from database.

    Args:
        limit: Optional row limit for testing
        connection: Optional SQLAlchemy connection/engine (for testing)
...
def temporal_split(
    df: pd.DataFrame, holdout_months: int = 1
    ...
    """
    # Ensure ts_month is datetime type (SQLite returns strings)
    df = df.copy()
    df["ts_month"] = pd.to_datetime(df["ts_month"])
