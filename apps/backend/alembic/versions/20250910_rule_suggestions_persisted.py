...
def upgrade():
    ...
    op.create_index("ix_anomaly_ignores_category", "anomaly_ignores", ["category"], unique=True)
    ...
    op.create_table(
        ...
        sa.Column("merchant", sa.String(255), nullable=False),
        sa.Column("category", sa.String(255), nullable=False),
        sa.Column("status", sa.String(16), nullable=False, server_default="new"),  # new|accepted|dismissed
    ...
    op.create_index("ix_rule_suggestions_persisted_status", "rule_suggestions_persisted", ["status"]) 
    op.create_index(
...
def downgrade():
    ...
    op.drop_index("ux_rule_suggestions_persisted_merchant_category", table_name="rule_suggestions_persisted")
    op.drop_index("ix_rule_suggestions_persisted_status", table_name="rule_suggestions_persisted")
    op.drop_table("rule_suggestions_persisted")
    op.drop_index("ux_rule_suggestion_ignores_merchant_category", table_name="rule_suggestion_ignores")
