...
def upgrade() -> None:
    ...
    with op.batch_alter_table("feedback", schema=None) as batch_op:
        existing_feedback_indexes = {
            ix["name"] for ix in inspector.get_indexes("feedback")
        }
        ...
        if batch_op.f("ix_feedback_txn_id") not in existing_feedback_indexes:
            batch_op.create_index(
                batch_op.f("ix_feedback_txn_id"), ["txn_id"], unique=False
            )
    ...
    if "rule_suggestions" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "support_count", sa.Integer(), server_default="0", nullable=False
            ),
    ...
    with op.batch_alter_table("rules", schema=None) as batch_op:
        ...
        if "target" in {c["name"] for c in inspector.get_columns("rules")}:
            batch_op.alter_column(
                "target", existing_type=sa.VARCHAR(length=32), nullable=True
            )
        if "category" in {c["name"] for c in inspector.get_columns("rules")}:
            batch_op.alter_column(
                "category", existing_type=sa.VARCHAR(length=128), nullable=False
            )
        batch_op.alter_column(
            "created_at",
            existing_type=sa.DATETIME(),
        ...
        if batch_op.f("ix_rules_merchant") not in existing_rules_indexes:
            batch_op.create_index(
                batch_op.f("ix_rules_merchant"), ["merchant"], unique=False
            )
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("user_labels", schema=None) as batch_op:
        ...
        batch_op.alter_column(
            "category", existing_type=sa.VARCHAR(length=128), nullable=True
        )
    ...
    with op.batch_alter_table("rules", schema=None) as batch_op:
        ...
        batch_op.alter_column(
            "target", existing_type=sa.VARCHAR(length=32), nullable=False
        )
        batch_op.alter_column(
            "pattern",
